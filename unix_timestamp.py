...
class UnixTimeStamp(CTk):
    def __init__(self):
        ...
        def show_timestamp() -> None:
            ...
            self._text_date_from_timestamp.configure(
                text="Текущая дата:" + str(users_date)
            )

        self._label_1 = CTkLabel(
            self, text="Unix Timestamp", font=("TimesNewRoman", 50)
        )
        self._title_label = CTkLabel(
            self, text="Текущий Unix timestamp:", font=("TimesNewRoman", 30)
        )
        self._text_timestamp = CTkLabel(
            self, text=f"{datetime.now().timestamp()}", font=("TimesNewRoman", 20)
        )
        self.field_user = CTkTextbox(self, height=40, width=300)
        self._text_date_from_timestamp = CTkLabel(
            self, text="Ваша дата:", font=("TimesNewRoman", 23)
        )
        self._button = CTkButton(
            self,
            text="ВЫВЕСТИ ВАШУ ДАТУ",
            command=show_timestamp,
            height=50,
            width=200,
